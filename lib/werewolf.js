...
async function pagi(conn, x, data) {
  ...
  return await conn.adReply(x.room, pagii(x), "W E R E W O L F", "", await conn.resize(thumb1, 300, 175), false)
}
...
async function voting(conn, x, data) {
  ...
  shortPlayer(x.room, data)
  for (let i = 0; i < x.player.length; i++) {
    textnya += `(${x.player[i].number}) ${conn.tagUser(x.player[i].id)} ${x.player[i].isdead === true ? "☠️" : ""}\n`
    ment.push(x.player[i].id)
  }
  ...
  return await conn.adReply(x.room, textnya, "W E R E W O L F", "", await conn.resize(thumb2, 300, 175), false)
}
...
async function malam(conn, x, data) {
  ...
  if (hasil_vote === 0) {
    ...
    return conn.adReply(x.room, textnya, "W E R E W O L F", "", await conn.resize(thumb3, 300, 175), false).then(() => {
      changeDay(x.room, data)
    ...
    if (hasil_vote.role === "werewolf") {
      textnya = `*⌂ W E R E W O L F - G A M E*\n\nWarga desa telah memilih dan sepakat ${conn.tagUser(hasil_vote.id)} dieksekusi mati.\n\n${conn.tagUser(hasil_vote.id)} adalah ${hasil_vote.role} ${emoji_role(hasil_vote.role)}`
      voteKill(x.room, data)
      let ment = []
      ment.push(hasil_vote.id)
      return await conn.adReply(x.room, textnya, "W E R E W O L F", "", await conn.resize(thumb4, 300, 175), false).then(() => {
        changeDay(x.room, data)
        voteDone(x.room, data)
        resetVote(x.room, data)
        clearAllVote(x.room, data)
...
async function skill(conn, x, data) {
  ...
  if (getWinner(x.room, data).status != null || x.win != null) {
    ...
    shortPlayer(x.room, data)
    for (let i = 0; i < x.player.length; i++) {
      tok1 += `(${x.player[i].number}) ${conn.tagUser(x.player[i].id)}${x.player[i].isdead === true ? " ☠️" : ""}\n`
      membernya.push(x.player[i].id)
    }
    for (let i = 0; i < x.player.length; i++) {
      tok2 += `(${x.player[i].number}) ${conn.tagUser(x.player[i].id)} ${x.player[i].role === "werewolf" || x.player[i].role === "sorcerer" ? `${x.player[i].isdead === true ? ` ☠️` : ` ${x.player[i].role}`}` : " "}\n`
      membernya.push(x.player[i].id)
    }
    for (let i = 0; i < x.player.length; i++) {
  ...
  if (getWinner(x.room, data).status === false || x.iswin === false) {
    ...
    for (let i = 0; i < x.player.length; i++) {
      if (x.player[i].role === "sorcerer" || x.player[i].role === "werewolf") {
        textnya += `${x.player[i].number}) ${conn.tagUser(x.player[i].id)}\n   *Role* : ${x.player[i].role}\n\n`
        ment.push(x.player[i].id)
      }
    }
    return await conn.adReply(sesinya, textnya, "W E R E W O L F", "", await conn.resize(thumb5, 300, 175), false).then(() => {
      delete data[x.room]
    })
    ...
    return await conn.adReply(sesinya, textnya, "W E R E W O L F", "", await conn.resize(thumb6, 300, 175), false).then(() => {
      delete data[x.room]
    })
  }
}
