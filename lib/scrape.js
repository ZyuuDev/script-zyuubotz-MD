...
import { searchPins } from "pinterest.js"
...
function tts(text, lang = 'id') {
  return new Promise((resolve, reject) => {
    try {
      let tts = gtts(lang)
      let filePath = join(Helper.__dirname(import.meta.url), '../tmp', (1 * new Date) + '.wav')
...
async function pinterest(query) {
    if (query.startsWith('https')) {
        ...
        try {
            const res = await searchPins(query)
            return res.response
        } catch (error) {
...
async function screenshotWeb(link, viewport = {}, timeout = 1000) {
    ...
    await page.screenshot({ path: './tmp/' + name, quality: 100, type: 'jpeg' })
    await browser.close()
    return fs.readFileSync("./tmp/" + name)
}
...
async function jadwalSholat(query) {
    try {
        // Fetch city data to get the city ID
        let response = await fetch(`https://api.myquran.com/v2/sholat/kota/cari/${query}`);
        ...
        const cityIds = cityData.data;
        const d = new Date().toLocaleDateString('id', { timeZone: 'Asia/Jakarta' }).split('/');
        const result = [];

        // Fetch prayer schedule for each city
        for (let city of cityIds) {
            try {
                let scheduleResponse = await fetch(`https://api.myquran.com/v2/sholat/jadwal/${city.id}/${d[2]}/${d[1]}/${d[0]}`);
                let scheduleData = await scheduleResponse.json();
                result.push({
                    lokasi: scheduleData.data.lokasi,
                    daerah: scheduleData.data.daerah,
                    jadwal: scheduleData.data.jadwal
                });
            } catch (err) {
                console.error(`Error fetching prayer schedule for city ID ${city.id}:`, err);
            }
        }
        ...
        console.error(`Error fetching city data for query ${query}:`, err);
        throw err;
    }
}
...
class Nimegami {
    ...
    async search(query) {
        const { data } = await axios.get(this.baseURL + `?s=${query}&post_type=post`)
        const $ = cheerio.load(data)
        let result = []
        $(".archive-a > article").each(function(a, b) {
            const thumbnail = $(b).find("img").attr("src")
            const title = $(b).find("h2").text()
            const status = $(b).find(".term_tag-a").text().trim()
            const link = $(".term_tag-a > a").attr("href")
            result.push({
                thumbnail,
                title,
                status,
    ...
    async detail(link) {
        ...
        const $ = cheerio.load(data)
        let result = []
        const thumbnail = $(".coverthumbnail > a > img").attr("src")
        $("tbody > tr").each(function(a, b) {
            const key = $(b).find("td").eq(0).text().replace(/:/gi, "").trim()
            const value = $(b).find("td").eq(1).text()
            result.push({
                key,
                value
            })
    ...
    async download(link) {
        const { data } = await axios.get(link)
        const $ = cheerio.load(data)
        const title = $(".subheading > h2").text()
        let batch = []
        let eps = []
        $(".download > ul").each(function(a, b) {
            const nameEps = `${title} Episode ${(a + 1).toString()}`
            let resolusi = []
            $(b).find("ul > li").each(function(c, d) {
                const name = $(d).find("a").attr("title")
                const resol = $(d).find("strong").text()
        ...
        $(".batch-dlcuy > ul").each(function(a, b) {
            const nameEps = title + " Batch"
            let resolusi = []
            $(b).find("ul > li").each(function(c, d) {
                const name = $(d).find("strong").text()
                const resol = $(d).find("strong").text()
                $(d).find("li > a").each(function(e, f) {
                    resolusi.push({
                        name,
