...
export default async function (m, conn = { user: {} }) {
  ...
  const senderKey = await checkUser(conn, m.sender)
  if (!senderKey) return

  let _name = await conn.getName(senderKey)
  let sender = senderKey.endsWith("lid") ? senderKey : PhoneNumber("+" + senderKey.replace("@s.whatsapp.net", "")).getNumber("international") + (_name ? " ~" + _name : "")
  ...
  let timestamp = m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date()
...
`.trim(),
    ...
    sender,
    m.exp || "?",
    user ? `|${user.exp}|${user.chat}|${user.command}` : "",
    m.chat + (chat ? " ~" + chat : ""),
    m.mtype
      ? m.mtype
          .replace(/message$/i, "")
