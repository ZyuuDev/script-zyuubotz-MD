let handler = async (m, { senderKey, usedPrefix, command }) => {
    global.db.data.bots.ultah[m.chat] = global.db.data.bots.ultah[m.chat] || {}
    if (typeof global.db.data.bots.ultah[m.chat][senderKey] === "undefined") return m.reply(`Kamu belum menambahkan hari ulang tahun kamu, silahkan tambahkan dahulu menggunakan command *${usedPrefix}setultah*`)
    let { tanggal, bulan, tahun } = global.db.data.bots.ultah[m.chat][senderKey]
