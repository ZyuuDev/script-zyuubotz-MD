...
export async function before(m, { conn, senderKey }) {
  ...
  if (!(id in conn.tebakbom) && m.quoted && /❏  *B O M B*|❏  *ʙ ᴏ ᴍ ʙ*/i.test(m.quoted.text)) return conn.reply(m.chat, `Sesi telah berakhir, silahkan kirim .tebakbom untuk membuat sesi baru.`, m)
  if (id in conn.tebakbom && !isNaN(body)) {
    ...
    if (json.emot == "💥") {
      ...
      conn.sendFile(m.chat, "https://telegra.ph/file/287cbe90fe5263682121d.jpg", "", teks, m)
      ...
      if (open >= 8) {
        ...
        conn.sendFile(m.chat, "https://telegra.ph/file/f6ebfea2758b947e1e49d.jpg", "", teks, m)
        global.db.data.users[senderKey].exp += reward
        clearTimeout(conn.tebakbom[id][2])
        delete conn.tebakbom[id]
      } else {
        let teks = `❏  *B O M B*\n\n`
