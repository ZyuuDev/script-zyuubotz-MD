...
let handler = async (m, { conn, command, usedPrefix, args }) => {
  ...
  if (value === "create") {
    ...
    for (let i = 0; i < ww[chat].player.length; i++) {
      text += `${ww[chat].player[i].number}) ${conn.tagUser(ww[chat].player[i].id)}\n`
      player.push(ww[chat].player[i].id)
    }
    text += "\nJumlah player minimal adalah 5 dan maximal 15"
    text += "\n\n_Silahkan Chat Bot terlebih dahulu sebelum memainkan game ini untuk menghindari spam!_"
    conn.adReply(m.chat, text.trim(), "W E R E - W O L F", "", await conn.resize(thumb, 300, 175), false, m)
    ...
    if (ww[chat].owner !== sender) return m.reply(`Hanya @${ww[chat].owner.split("@")[0]} yang dapat memulai permainan`, false, { mentions: [ww[chat].owner] })
    let list1 = ""
    let list2 = ""
    let player = []
    ...
    startGame(chat, ww)
    for (let i = 0; i < ww[chat].player.length; i++) {
      list1 += `(${ww[chat].player[i].number}) ${conn.tagUser(ww[chat].player[i].id)}\n`
      player.push(ww[chat].player[i].id)
    }
    for (let i = 0; i < ww[chat].player.length; i++) {
      list2 += `(${ww[chat].player[i].number}) ${conn.tagUser(ww[chat].player[i].id)} ${ww[chat].player[i].role === "werewolf" || ww[chat].player[i].role === "sorcerer" ? `[${ww[chat].player[i].role}]` : ""}\n`
      player.push(ww[chat].player[i].id)
    }
    await conn.adReply(m.chat, "*⌂ W E R E W O L F - G A M E*\n\nGame telah dimulai, para player akan memerankan perannya masing masing, silahkan cek chat pribadi untuk melihat role kalian. Berhati-hatilah para warga, mungkin malam ini adalah malah terakhir untukmu", "W E R E - W O L F", "", await conn.resize(thumb, 300, 175), false, m)
    for (let i = 0; i < ww[chat].player.length; i++) {
      // [ Werewolf ]
    ...
    for (let i = 0; i < ww[chat].player.length; i++) {
      text += `(${ww[chat].player[i].number}) ${conn.tagUser(ww[chat].player[i].id)} ${ww[chat].player[i].isdead === true ? `☠️ ${ww[chat].player[i].role}` : ""}\n`
      player.push(ww[chat].player[i].id)
    }
    conn.adReply(m.chat, text, "W E R E - W O L F", "", await conn.resize(thumb, 300, 175), false, m)
  } else {
