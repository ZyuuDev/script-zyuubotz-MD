...
export async function before(m, { conn, senderKey }) {
  ...
  } else if (room.curr !== senderKey) {
    if (!m.quoted || !m.quoted.fromMe || !/(Mulai|Tersisa) ?:|(ᴍᴜʟᴀɪ|ᴛᴇʀꜱɪꜱᴀ) ?:/i.test(m.quoted.text)) return !0
    if (m.quoted.id == room.chat.id) {
      if (room.eliminated.includes(senderKey)) m.reply(`_Hei, kamu sudah tereliminasi, tunggu hingga game ini selesai_\n*Nice Try, next game*`)
      else if (room.player.includes(senderKey)) {
