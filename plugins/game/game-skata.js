...
╚═════`.trim()
let poin = 500
let handler = async (m, { conn, text, isPrems, isROwner, usedPrefix, command, senderKey }) => {
    let isDebug = /debug/i.test(command) && isROwner
    conn.skata = conn.skata ? conn.skata : {}
    let id = m.chat
    let kata = await genKata()
    let room_all = Object.values(conn.skata).find(room => room.id !== id && room.player.includes(senderKey))
    ...
    if (id in conn.skata) {
        ...
        if (room.status == 'play') {
            if (!room.waktu._destroyed && !room.diam) return conn.reply(m.chat, `Hii @${senderKey.split`@`[0]}, Masih ada game berlangsung di chat ini\nTunggu hingga game berakhir\nLalu ikut bergabung`, room.chat, { contextInfo: { mentionedJid: [senderKey] } })
            delete conn.skata[id]
        }
        if (text == 'start' && room.status == 'wait') {
            if (!member.includes(senderKey)) return m.reply('Kamu belum ikut')
            if (member.length < 2) return m.reply(`Minimal 2 orang`)
            room.curr = member[0]
            room.status = 'play'
            ...
            if (member.includes(senderKey)) return m.reply(`Kamu sudah ikut di list`)
            member.push(senderKey)
            clearTimeout(room.waktu_list)
...
Dan hanya bisa dimainkan oleh player yang terdaftar
`.trim()
