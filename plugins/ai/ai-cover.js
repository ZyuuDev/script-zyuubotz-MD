...
`.trim())
    if (typeof conn.coverai[senderKey] !== "undefined") return m.reply("Audiomu masih diproses!")
    await conn.loading(m, conn)
    conn.coverai[senderKey] = "proses"
    let media = await q.download()
    let cover = await aiCover(char, media)
    await conn.sendFile(m.chat, cover.base64, "", "", m, false, { mimetype: "image/jpeg" })
  } finally {
    await conn.loading(m, conn, true)
    delete conn.coverai[senderKey]
  }
}
