...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        ...
        let imageUrl = global.API('botcahx', '/api/maker/jadihitam', { url: files }, 'apikey')
        await conn.sendMessage(m.chat, { image: { url: imageUrl }, fileName: 'jadihitam.jpg', mimetype: 'image/jpeg', caption: setting.smlcap ? conn.smlcap("Ini Dia Kak") : "Ini Dia Kak" }, { quoted: m, ephemeralExpiration: ephemeral })
    } finally {
...
handler.tags = ['tools']
handler.command = /^(jadi|to)?jadihitam|h(y|i)tam$/i
handler.limit = true
