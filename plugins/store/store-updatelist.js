...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (typeof store[cmd] === "undefined") return m.reply("Command tidak ada!")
  ...
  if (/image\/(jpe?g|png)|video\/mp4/.test(mime)) {
    ...
    m.reply(`Sukses meng-update command ${cmd} kedalam menu store`)
    store[cmd] = {
      command: cmd,
      media: files,
...
handler.tags = ["store"]
handler.command = /^(update(store|list))$/i
