import { mediafiredl } from '../../lib/scrape.js'

let handler = async (m, { conn, args, usedPrefix, command, isPrems, senderKey }) => {
  ...
  if (senderKey in conn.mediafire) return m.reply('Kamu Masih Mendownload!')
  if (!/https:\/\/www.mediafire.com\//i.test(args[0])) return m.reply('InvalsenderKey Url!')

  try {
    conn.mediafire[senderKey] = true
...
`.trim()
    ...
    delete conn.mediafire[senderKey]
  }
}
