...
Contoh Penggunaan:
> Untuk Membeli Saham
> ${usedPrefix}saham-buy bitcoin 1
> Untuk Menjual Saham
> ${usedPrefix}saham-sell bitcoin 1
`.trim()
  ...
  let coinEntry = Object.entries(bot.saham.item).find(([key, val]) => (val.name.toLowerCase() === coinName || key === coinName) && !val.suspend)

  if (!coinEntry) {
    return m.reply(`Nama koin tidak ditemukan!\n*List koin:* \n\n${saham.map((v) => `*•* ${v[1].name}`).join("\n")}`)
  }

  let [coinKey, coinData] = coinEntry
  let { harga } = coinData

  if (!user.saham[coinKey]) {
    user.saham[coinKey] = { harga: 0, stock: 0 }
  }
  ...
  switch (commands) {
    case "buy": {
      let price = harga * total * 100
      if (price > user.bank) return m.reply("Saldo bank kamu kurang untuk membeli koin ini")

      let average = await calculateAverage(user.saham[coinKey].harga, user.saham[coinKey].stock, harga, total)
      user.bank -= price
      user.saham[coinKey].stock += total * 100
      user.saham[coinKey].harga = average

      coinData.marketcap += total * 100
      coinData.volumeBuy += total * 100
      coinData.trade.push({ user: senderKey, total, type: "buy" })

      m.reply(`Berhasil membeli ${total} lot ${capitalize(coinKey)} ${coinData.symbol} seharga ${toRupiah(price)} Bank ${emot("bank")}`)
      break
    }
    case "sell": {
      if (user.saham[coinKey].stock < total * 100) {
        return m.reply(`Kamu hanya memiliki ${user.saham[coinKey].stock} ${capitalize(coinKey)} ${coinData.symbol}`)
      }
      ...
      coinData.marketcap -= total * 100
      coinData.volumeSell += total * 100
      coinData.trade.push({ user: senderKey, total, type: "sell" })

      m.reply(`Berhasil menjual ${total} lot ${capitalize(coinKey)} ${coinData.symbol} seharga ${toRupiah(price)} Bank ${emot("bank")}`)
      break
    }
    case "history": {
