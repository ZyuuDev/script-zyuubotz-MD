let buatall = 1
let handler = async (m, { senderKey, conn, args, usedPrefix, DevMode }) => {
    ...
    try {
        ...
        let count = args[0]
        count = count ? /all/i.test(count) ? Math.floor(global.db.data.users[senderKey].exp / buatall) : parseInt(count) : args[0] ? parseInt(args[0]) : 1
        count = Math.max(1, count)
        if (args.length < 1) return conn.reply(m.chat, usedPrefix + 'casino <jumlah>\n ' + usedPrefix + 'casino 1000', m)
        if (global.db.data.users[senderKey].exp >= count * 1) {
            global.db.data.users[senderKey].exp -= count * 1
            //await m.reply('') //Kwkwwkkwlwlw
            if (Aku > Kamu) {
                conn.reply(m.chat, `💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You LOSE*\nKamu kehilangan ${toRupiah(count)} Uang(xp)`.trim(), m)
            } else if (Aku < Kamu) {
                global.db.data.users[senderKey].exp += count * 2
