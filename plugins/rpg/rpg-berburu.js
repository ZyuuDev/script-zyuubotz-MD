let handler = async (m, { senderKey, conn }) => {
    let __timers = (new Date - global.db.data.users[senderKey].lastmisi)
    let _timers = (3600000 - __timers)
    let timers = clockString(_timers)
    let name = conn.getName(senderKey)
    ...
    if (new Date - user.lastmisi > 3600000) {
        ...
        let hsl = `🕸 *Hasil buruan ${user.registered ? user.name: conn.getName(senderKey)}*
        ${hewan1 ? `
