let handler = async (m, { senderKey, conn, text }) => {
    ...
    let player = global.db.data.users[senderKey]
    ...
    if (timeSinceLastHunt > 120000) {
        ...
        if (player.health < 0) {
            ...
            let deathMessage = `*@${senderKey.split("@")[0]}* Anda Mati Dibunuh Oleh ${monsterName}`
            if (player.level > 0) {
