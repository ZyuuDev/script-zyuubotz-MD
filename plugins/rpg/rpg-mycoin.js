...
let handler = async (m, { conn, senderKey }) => {
    ...
    let cryptoBot = bot.invest.item

    // Filter hanya crypto yang tidak disuspend
    let activeItems = Object.entries(cryptoBot)
        .filter(([key, val]) => val.suspend === false)
        .map(([key]) => key)

    // Ambil data crypto user yang valid dan aktif
    let invest = Object.entries(user.invest || {})
        .filter(([key, val]) => val.stock > 0 && activeItems.includes(key))

    if (invest.length === 0) {
        return conn.reply(m.chat, `Kamu belum memiliki crypto aktif saat ini.`, m)
    }

    const hargaSebelumnyas = await Hitung(invest)
    const hargaSekarangs = await HitungProfit(invest, cryptoBot)
...
async function HitungProfit(invest, cryptoBot) {
    return invest.reduce((acc, [key, val]) => acc + (val.stock * cryptoBot[key].harga), 0)
}
