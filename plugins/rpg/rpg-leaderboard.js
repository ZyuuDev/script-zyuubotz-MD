...
const leaderboards = ["atm", "level", "exp", "money", "iron", "gold", "diamond", "emerald", "trash", "potion", "wood", "rock", "string", "umpan", "petfood", "common", "uncommon", "mythic", "legendary", "pet", "bank", "chip", "skata", "donasi", "deposit", "garam", "minyak", "gandum", "steak", "ayam_goreng", "ribs", "roti", "udang_goreng", "bacon"]

leaderboards.sort((a, b) => a.localeCompare(b))

let handler = async (m, { senderKey, conn, data, args, participants, usedPrefix, command }) => {
...
${usedPrefix}${command} legendary`.trim()
  ...
  let dataUser = await Promise.all(
    sortedItem.slice(page * 0, 1 * 5 + 5).map(async (user, i) => {
      return {
        top: i + 1,
  ...
  let text = `
🏆 rank: ${toRupiah(userItem.indexOf(senderKey) + 1)} out of ${toRupiah(userItem.length)}

                *• ${global.rpg.emoticon(type)} ${type} •*

${sortedItem.slice(page * 0, page * 5 + 5).map((user, i) => `${i + 1}.*﹙${toRupiah(user[type])}﹚*- ${participants.some((p) => (p.phoneNumber || p.id) === conn.decodeJid((p.phoneNumber || p.id).endsWith("@s.whatsapp.net") ? user.jid : user.lid)) ? `${user.registered ? user.name : conn.getName(user.jid)} \nwa.me/` : "from other group\n @"}${user.jid.split`@`[0]}`).join`\n\n`}
`.trim()
