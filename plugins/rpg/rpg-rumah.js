...
let handler = async (m, { senderKey, conn, usedPrefix, command, text }) => {
    ...
    let who = m.mentionedJid[0] ? m.mentionedJid[0] : senderKey

    const quotedKey = await checkUser(conn, who)

    let name = user[senderKey].registered ? user[senderKey].name : await conn.getName(senderKey)
    ...
    switch (command) {
        case 'rumah-buy': {
            ...
            if (kondisi && id) {
                if (kondisi == "bekas") {
                    ...
                    if (item.seller == senderKey) return m.reply("Tidak bisa membeli rumah sendiri")
                    if (user[senderKey].bank < item.harga) return m.reply(`Uang kamu tidak cukup untuk membeli rumah ini!`)
                    if (typeof user[senderKey].rumah[item.id] !== "undefined") return m.reply("Tidak bisa membeli rumah yang sama!")
                    ...
                    user[senderKey].rumah[item.id] = {
                        id: item.id,
                        harga: item.harga,
                    }
                    user[senderKey].bank -= item.harga
        ...
        case "rumah-sell": {
            let userRumah = Object.values(user[senderKey].rumah)
        ...
        default: {
            let userRumah = Object.values(user[quotedKey].rumah)
            if (!userRumah.length > 0) return m.reply(`${quotedKey == senderKey ? "Kamu" : "Dia"} tidak mempunyai rumah! \n_Gunakan command *${usedPrefix}rumah-buy* untuk membeli rumah_`)
            let caption = `
*RUMAH ${quotedKey == senderKey ? "KAMU" : "DIA"}*
...
`.trim()
            await conn.adReply(m.chat, caption, `Halo ${name}, ${wish()}`, global.config.watermark, fs.readFileSync("./media/rumah.jpg"), global.config.website, m)
        }
    }
}
