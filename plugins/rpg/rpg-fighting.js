let handler = async (m, { senderKey, conn, usedPrefix, participants }) => {
    conn.fight = conn.fight ? conn.fight : {}
    let user = global.db.data.users[senderKey]

    if (typeof conn.fight[senderKey] != "undefined" && conn.fight[senderKey] == true) return m.reply(`Kamu masih bertarung.`)
    let users = participants.map(u => u.id)
    var lawan
    lawan = users[Math.floor(users.length * Math.random())]

    while (typeof global.db.data.users[lawan] == "undefined" || lawan == senderKey) {
        lawan = users[Math.floor(users.length * Math.random())]
    }
    ...
    conn.fight[senderKey] = true

    await delay(300000)

    let kesempatan = []
    for (let i = 0; i < user.level; i++) kesempatan.push(senderKey)
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (kesempatan[unggul] == senderKey) pointPemain += 1
        else pointLawan += 1
    }
