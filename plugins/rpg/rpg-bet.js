...
Kamu *${status}*, kamu ${status == 'Menang' ? `Mendapatkan *+${toRupiah(count * 2)}*`: status == 'Kalah' ? `Kehilangan *-${toRupiah(count * 1)}*`: `Mendapatkan *+${toRupiah(Math.floor(count / 1.5))}*`} *${type}* ${global.rpg.emoticon(type)}
`.trim())
    } catch (e) {
