...
let handler = async (m, { senderKey, conn, usedPrefix, text }) => {
  let users = global.db.data.users
  if (text) {
    if ('ref_count' in users[senderKey]) return m.reply('Tidak bisa menggunakan kode referal!')
    let link_creator = (Object.entries(users).find(([, { ref_code }]) => ref_code === text.trim()) || [])[0]
    if (!link_creator) return m.reply('Kode referal tidak valid')
    let count = users[link_creator].ref_count++
    let extra = xp_bonus[count] || 0
    users[link_creator].exp += xp_link_creator + extra
    users[senderKey].exp += xp_first_time
    users[senderKey].ref_count = 0
    m.reply(`
Selamat!
...
`.trim(), link_creator)
    ...
    let code = users[senderKey].ref_code = users[senderKey].ref_code || new Array(11).fill().map(() => [...'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'][crypto.randomInt(62)]).join('')
    users[senderKey].ref_count = users[senderKey].ref_count ? users[senderKey].ref_count : 0
...
${users[senderKey].ref_count} orang telah menggunakan kode referal kamu
