...
let handler = async (m, { conn, usedPrefix, command, text, senderKey }) => {
  ...
  if (targetUser === conn.user.jid) {
    return m.reply("❌ Tidak bisa promote bot!")
  }
  ...
  try {
    ...
    await m.reply(`✅ Berhasil mempromosikan @${targetUser.split("@")[0]} menjadi admin!`, false, { mentions: [targetUser] })
  } catch (error) {
    console.error("Error promoting user:", error)
    await m.reply(`❌ Gagal mempromosikan user. Pastikan bot adalah admin grup.`)
  }
}
