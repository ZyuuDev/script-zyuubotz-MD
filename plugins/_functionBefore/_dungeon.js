...
export async function before(m, { conn, senderKey }) {
  ...
  let room = Object.values(conn.dungeon).find((room) => room.id.startsWith("dungeon-") && [room.game.player1, room.game.player2, room.game.player3, room.game.player4].includes(senderKey) && room.state == "WAITING")
  if (room) {
